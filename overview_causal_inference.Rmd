...
???
Sort the table by increasing `p.value` (click `p.value`)
...
Tests for independence are linear in the dagitty package. Distance Correlation can detect non-linear correlation. For example:

```{r, eval=FALSE} 
library(energy) # provides tests for marginal and partial independence using distance correlation 

# Tests show probability of independence
# Low p values indicate dependence

# Conditional distance correlation is computationally intensive O(n^2) and memory intensive, so use a random sample of data
...
pdcor.test(monicasamp$bmigrp, monicasamp$weight, monicasamp$bmi, R=99) # Dependent although model indicates independence
