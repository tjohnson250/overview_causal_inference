...
pgmonica <- ggdag(gmonica, layout="circle") + theme_dag_gray()
pgmonica
```
]
...
rlowp <- r[r$p.value<0.05,] 
pr <- ggplot(rlowp, aes(x=estimate, y=rownames(rlowp))) + geom_point() + geom_errorbarh(mapping=aes(y=rownames(rlowp), xmin=!!sym("2.5%"), xmax = !!sym("97.5%")), height=0) +theme_bw(base_size = 15)
pr
```
]
???
...
```{R, echo=FALSE}
# dagitty does not return the basis set required for Fisher's C
# so we define functions based on Shipley, 2000 that return
# the correct basis set
library(dagitty)
basisSet <- function(dag, v) {
  vars <- unique(tidy_dagitty(gmonica)$data$name)
  vars2 <- vars[!vars %in% c(adjacentNodes(gmonica, v), v)]
  lapply(vars2, function(v2) list(X = v, Y=v2, Z=unique(c(parents(dag, v), parents(dag, v2)))))
}
shipleyBasisSet <- function(dag) {
  vars <- unique(tidy_dagitty(gmonica)$data$name)
  result <- unlist(lapply(vars, function(v) basisSet(gmonica, v)), recursive=FALSE)
  return(result)
}

gmonicaBasisSet <- shipleyBasisSet(gmonica)

r = localTests(gmonica, monica, tests=gmonicaBasisSet)
```
...
model <- '
  # regressions
  height ~ eage + sex
  weight ~ height + eage + sex
  bmi ~ weight + height
  hychol ~ bmi + sex + eage + chdt + chrx
  chdt ~ high
  chrx ~ high
  '
fit <- sem(model, data=monica, std.ov=TRUE) # , ordered = c("hychol")
```
...
# switch this to corrr to remove low res plot: https://drsimonj.svbtle.com/exploring-correlations-in-r-with-corrr
...
plot_matrix(residuals(fit, type="cor")$cov) # 
#plot_matrix(residuals(fit))
```
---
exclude: false
...
The highest MI involved adding an undirected edge between $chdt$ and $chrx$, indicating an unmeasured common cause.
...
---
# Part IV Learning Objectives: Causal Discovery from Observational Data

- Describe how conditional independence tests of a dataset constrains the possible causal DAGs that may have generated the data
- Describe the steps of the PC causal discovery algorithm
...
# PC Algorithm.fn[1]
1. Adjacency Phase
    - Determine which variables are adjacent
1. Orientation Phase
    - Orient direction of arrows if possible

.footnote[[1] Spirtes and Glymour, 1991]

---
### PC Algorithm: Adjacency Phase

$X$ and $Y$ are not adjacent if they are conditionally independent given any subset of variables that does not contain $X$ and $Y$

- Begin with a fully connected graph
- Remove edge between $X$ and $Y$ if $X \perp\!\!\!\perp Y | \text{ any set } S$
    - Begin with the empty set and test all connected pairs for marginal independence $X \perp\!\!\!\perp Y$
        - Remove edges for any pair that is independent 
    - For any remaining connected pair, test conditional independence vs. each single other variable, ie., $S$ of size 1
        - Remove edge between $X$ and $Y$ if $X \perp\!\!\!\perp Y | S \text{ of size 1}$
    - Repeat for $S$ of size 2, 3, etc.

---
background-image: url(PCAlignmentPhaseExample.png)
background-size: contain
.footnote[From https://www.ccd.pitt.edu/wp-content/uploads/2017/02/CCD_Summer_Course_2016_Day3_morning.pdf]

---
background-image: url(PCOrientationRules.png)
background-size: contain
.footnote[From https://www.ccd.pitt.edu/wp-content/uploads/2017/02/CCD_Summer_Course_2016_Day3_morning.pdf]
???
X -- Y -- Z, X and Z must not be adjacent. If Y is not in S, then X and Z, then Y is a collider for X and Z. If Y separates X and Z, then we cannot orient the edges (other than to say that Y is not a collider)

---
background-image: url(PCOrientationPhaseExample.png)
background-size: contain
.footnote[From https://www.ccd.pitt.edu/wp-content/uploads/2017/02/CCD_Summer_Course_2016_Day3_morning.pdf]

---
exclude: true
## PC Algorithm: Adjacency Phase Example
.pull-left[
Generative DAG
```{r, echo=FALSE}
knownDAG <- dagitty('dag{
  X1 [pos="0,2"]
  X2 [pos="0,0"]
  X3 [pos="1,1"]
  X4 [pos="2,1"]
  ...
  X2 -> X3
  X3 -> X4}')
ggdag(knownDAG, text_size=4)  + theme_dag_blank()
```
...
fcDAG <- dagitty('dag{
  ...
  X2 <-> {X3 X4}
  X3 <-> X4}')
ggdag(fcDAG, text_size=4) + theme_dag_blank()
```
]
...
.pull-left[
```{r, echo=FALSE, fig.height=6}
pcmonica <- monica %>% select(-bmigrp) %>% select(-hychol) # Remove bmigrp and hychol to match earlier model
suffStat <- list(C = cor(pcmonica), n = nrow(pcmonica))
pc.monica <- pc(suffStat, indepTest = gaussCItest, labels = names(pcmonica), alpha = 0.01, skel.method="stable", maj.rule = TRUE)
plot(pc.monica)
```
]
...
```{r, echo=FALSE}
pcmonica <- monica %>% select(-bmigrp) %>% select(-hychol) # Remove bmigrp and hychol to match earlier model
res <- tetradrunner(algoId = "pc-all", df = pcmonica, alpha = .01, testId = "fisher-z", dataType = 'continuous', stableFAS = TRUE, colliderDiscoveryRule = 2)
...
```{r, echo=FALSE}
pcmonica <- monica %>% select(-bmigrp) %>% select(-hychol) # Remove bmigrp and hychol to match earlier model
res <- tetradrunner(algoId = "pc-all", df = pcmonica, alpha = .01, testId = "fisher-z", dataType = 'continuous', stableFAS = TRUE, priorKnowledge = prior, verbose = TRUE, colliderDiscoveryRule = 2)
graph_dot <- tetradrunner.tetradGraphToDot(res$graph)
grViz(graph_dot)
```
]
