...
institute: "UTHealth School of Biomedical Informatics" 
date: "`r Sys.Date()`"
output:
...
---
### Results of the PC Algorithm (Spirtes et al. 1991) on Who Monica Dataset 

.pull-left[
```{r, echo=FALSE, fig.height=6}
suffStat <- list(C = cor(monica), n = nrow(monica))
pc.monica <- pc(suffStat, indepTest = gaussCItest, labels = names(monica), alpha = 0.05, skel.method="stable")
plot(pc.monica)
```
]
...
# tetrad PC algo
```{r}
res <- tetradrunner(algoId = "pc-all", df = monica, alpha = .05, testId = "fisher-z", dataType = 'continuous', stableFAS = TRUE)
library(DOT)
